...
import { X, Loader2, MapPin, Upload, RefreshCw } from 'lucide-react';
...
export default function ReportIncidentModal({ isOpen, onClose, onSubmit }: ReportIncidentModalProps) {
  ...
  useEffect(() => {
    ...
    // If we don't have high accuracy, request it
    if (!isHighAccuracy) {
      requestHighAccuracy();
    }
    ...
    if (locationToUse) {
      getLocationDetails(locationToUse.lat, locationToUse.lng)
        .then(zone => {
          setLocation({
  ...
  }, [isOpen, currentLocation, lastKnownLocation, isHighAccuracy]);
      ...
      <DialogContent className="sm:max-w-md bg-white h-[calc(100dvh-16px)] sm:h-auto mt-2 mb-2 mx-2 sm:m-auto overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 pb-20 sm:pb-4">
          ...
          <div className="flex items-center space-x-2 text-sm">
            ...
            {isHighAccuracy ? (
              <Button onClick={handleRefreshLocation} className="text-gray-600 hover:text-gray-500">
          ...
          <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t sm:relative sm:border-t-0 sm:p-0 sm:bg-transparent">
            <button
              type="submit"
              disabled={!location || !isHighAccuracy || isSubmitting}
